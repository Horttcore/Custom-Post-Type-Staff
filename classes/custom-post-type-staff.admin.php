...
final class Custom_Post_Type_Staff_Admin
{
    ...
    public function __construct()
    {
        ...
        add_action('manage_staff_posts_custom_column', [$this, 'manage_staff_posts_custom_column'], 10, 2);
        add_action('save_post', [$this, 'staff_save_metabox']);
        add_action('enqueue_block_editor_assets', [$this, 'register_block']);

        add_filter('manage_edit-staff_columns', [$this, 'manage_edit_staff_columns']);
    ...
    {
        add_meta_box('staff-meta', __('Information', 'custom-post-type-staff'), [$this, 'staff_meta'], 'staff', 'normal');
    }
    ...
    public function staff_meta($post)
    {
        ...
        <table class="form-table">
            ...
            <tr>
                ...
                <td>
                    <label><input <?php if ($meta) {
            checked('mr', $meta['salutation']);
        } ?> type="radio" value="mr" name="staff-salutation" id="staff-salutation-male"> <?php _e('Mr', 'custom-post-type-staff'); ?></label><br>
                    <label><input <?php if ($meta) {
            checked('ms', $meta['salutation']);
        } ?> type="radio" value="ms" name="staff-salutation" id="staff-salutation-female"> <?php _e('Ms', 'custom-post-type-staff'); ?></label>
                </td>
            </tr>
            <tr>
                <th><label for="staff-grade"><?php _e('Grade:', 'custom-post-type-staff'); ?></label></th>
                <td><input size="50" type="text" value="<?php if ($meta) {
            echo esc_attr($meta['grade']);
        } ?>" name="staff-grade" id="staff-grade"></td>
            </tr>
            <tr>
                <th><label for="staff-first-name"><?php _e('First name:', 'custom-post-type-staff'); ?></label></th>
                <td><input size="50" type="text" value="<?php if ($meta) {
            echo esc_attr($meta['first-name']);
        } ?>" name="staff-first-name" id="staff-first-name"></td>
            </tr>
            <tr>
                <th><label for="staff-last-name"><?php _e('Last name:', 'custom-post-type-staff'); ?></label></th>
                <td><input size="50" type="text" value="<?php if ($meta) {
            echo esc_attr($meta['last-name']);
        } ?>" name="staff-last-name" id="staff-last-name"></td>
            </tr>
            <tr>
                <th><label for="staff-role"><?php _e('Role:', 'custom-post-type-staff'); ?></label></th>
                <td><input size="50" type="text" value="<?php if ($meta) {
            echo esc_attr($meta['role']);
        } ?>" name="staff-role" id="staff-role"></td>
            </tr>
            <tr>
                <th><label for="staff-room"><?php _e('Room:', 'custom-post-type-staff'); ?></label></th>
                <td><input size="50" type="text" value="<?php if ($meta) {
            echo esc_attr($meta['room']);
        } ?>" name="staff-room" id="staff-room"></td>
            </tr>
            <tr>
                <th><label for="staff-phone"><?php _e('Phone:', 'custom-post-type-staff'); ?></label></th>
                <td><input size="50" type="text" value="<?php if ($meta) {
            echo esc_attr($meta['phone']);
        } ?>" name="staff-phone" id="staff-phone"></td>
            </tr>
            <tr>
                <th><label for="staff-fax"><?php _e('Fax:', 'custom-post-type-staff'); ?></label></th>
                <td><input size="50" type="text" value="<?php if ($meta) {
            echo esc_attr($meta['fax']);
        } ?>" name="staff-fax" id="staff-fax"></td>
            </tr>
            <tr>
                <th><label for="staff-mobile"><?php _e('Mobile:', 'custom-post-type-staff'); ?></label></th>
                <td><input size="50" type="text" value="<?php if ($meta) {
            echo esc_attr($meta['mobile']);
        } ?>" name="staff-mobile" id="staff-mobile"></td>
            </tr>
            <tr>
                <th><label for="staff-email"><?php _e('E-Mail:', 'custom-post-type-staff'); ?></label></th>
                <td><input size="50" type="text" value="<?php if ($meta) {
            echo esc_attr($meta['email']);
        } ?>" name="staff-email" id="staff-email"></td>
            </tr>
            <tr>
                <th><label for="staff-url"><?php _e('URL:', 'custom-post-type-staff'); ?></label></th>
                <td><input size="50" type="text" value="<?php if ($meta) {
            echo esc_url($meta['url']);
        } ?>" name="staff-url" id="staff-url"></td>
            </tr>
